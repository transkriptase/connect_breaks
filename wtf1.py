...
def connect_broken_tracks(broken_tracks, not_real_tracks, frame_threshold, distance_threshold, radius, filled_locations, track_names, frame_count):
    ...
    while remaining_broken_tracks:
        ...
        for track1_name, (start1, end1) in list(remaining_broken_tracks.items()):
            ...
            if best_candidate:
                ...
                if track1_name in track_chain:
                    track_chain[track1_name] += f"_{best_candidate[0]}"
                else:
                    track_chain[track1_name] = f"{track1_name}_{best_candidate[0]}"
                if best_candidate[2] == frame_count - 1:  # If the connected track ends at end_frame
                    completed_tracks.append(track_chain[track1_name])
    ...
    return connections, completed_tracks, track_chain

def create_new_tracks(connections, track_chain):
    new_tracks = {}
    for track1_name, start1, end1, track2_name, start2, end2 in connections:
        new_track_name = track_chain.get(track1_name, f"{track1_name}_{track2_name}")
...
if __name__ == "__main__":
    ...
    connected_tracks, completed_tracks, track_chain = connect_broken_tracks(broken_tracks, not_real_tracks, frame_threshold, distance_threshold, radius, filled_locations, track_names, frame_count)
    ...
    new_tracks = create_new_tracks(connected_tracks, track_chain)
    print("NEW TRACKS")
