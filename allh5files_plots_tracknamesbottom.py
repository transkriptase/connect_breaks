...
if __name__ == "__main__":
    for filename in os.listdir(directory):
        if filename.endswith(".h5"):
            ...
            # Filter not real tracks with a frame length less than 10
            ...
            for track_name in track_names:
                ...
                plt.plot(x_data, y_data, label=track_name)
            plt.title(f"Tracks for {filename}")
            plt.xlabel("X position")
            plt.ylabel("Y position")
            plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=4, fontsize='small')  # Position the legend at the bottom
            plt.tight_layout(rect=[0, 0.03, 1, 1])  # Adjust the layout to make space for the legend
